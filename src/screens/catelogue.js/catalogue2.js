...
import { View, Text, SafeAreaView, FlatList, Image, TouchableOpacity, Modal, Button, StyleSheet } from 'react-native'
import Header from '../../components/header'
import ImagePicker from 'react-native-image-crop-picker';
import { useDispatch, useSelector } from 'react-redux';
import { AddToFavorite, storeImagePot } from '../../redux/actions/app';

const Catalogue2 = ({ route }) => {
    const { name } = route.params.item
    const { storedImagePot } = useSelector(state => state.appDetails)

    const dispatch = useDispatch()
    const [images, setImages] = useState(storedImagePot)
    ...
    const ImageSelector = () => {
        ImagePicker.openPicker({ multiple: true })
            .then(selectedImage => {
                ...
                dispatch(storeImagePot([...images, ...uploadedImages]))
    ...
    const OpenCamera = () => {
        ...
        ImagePicker.openCamera({
            ...
            console.log(data);
            dispatch(storeImagePot([...images, data]))
